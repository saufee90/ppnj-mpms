    ...
    <form method="POST" action="{{ route('data-harian.update', $daily_operation) }}" class="space-y-6">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="tarikh" value="{{ old('tarikh', $daily_operation->tarikh->toDateString()) }}" max="{{ now()->format('Y-m-d') }}" required class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="operation_status" id="operation_status" required class="w-full border rounded-lg px-3 py-2 text-sm">
                        ...
                        <option value="tidak_operasi_terima_bts" {{ old('operation_status', $daily_operation->operation_status ?? 'operasi') === 'tidak_operasi_terima_bts' ? 'selected' : '' }}>Terima BTS Sahaja (Tidak Proses)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="mt-4 non-operasi-only-field hidden">
                ...
                <textarea name="operation_note" rows="2" class="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Contoh: Kilang terima BTS sahaja, proses akan dibuat hari berikutnya.">{{ old('operation_note', $daily_operation->operation_note) }}</textarea>
            </div>
        </div>
