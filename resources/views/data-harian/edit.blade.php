...
<div class="bg-white rounded-xl shadow-sm p-6 max-w-4xl">
    <form id="daily-operation-form" method="POST" action="{{ route('data-harian.update', $daily_operation) }}" class="space-y-6" data-form-mode="edit">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="tarikh" max="{{ now()->toDateString() }}" value="{{ old('tarikh', $daily_operation->tarikh->toDateString()) }}" required class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    ...
                    <select name="mill_id" required class="w-full border rounded-lg px-3 py-2 text-sm">
                        @foreach($mills as $mill)
                            <option value="{{ $mill->id }}" data-code="{{ $mill->code }}" {{ old('mill_id', $daily_operation->mill_id) == $mill->id ? 'selected' : '' }}>{{ $mill->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="operation_status" required class="w-full border rounded-lg px-3 py-2 text-sm">
                        <option value="Operasi" {{ old('operation_status', $daily_operation->operation_status ?? 'Operasi') === 'Operasi' ? 'selected' : '' }}>Operasi</option>
                        <option value="Tidak Operasi (Terima Buah Sahaja)" {{ old('operation_status', $daily_operation->operation_status ?? 'Operasi') === 'Tidak Operasi (Terima Buah Sahaja)' ? 'selected' : '' }}>Tidak Operasi (Terima Buah Sahaja)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="baki_bts_semalam" value="{{ old('baki_bts_semalam', $daily_operation->baki_bts_semalam) }}" @readonly(!($canEditBakiBtsSemalam ?? false)) class="w-full border rounded-lg px-3 py-2 text-sm {{ !($canEditBakiBtsSemalam ?? false) ? 'bg-gray-100' : '' }}">
                </div>
                <div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="produksi_cpo" value="{{ old('produksi_cpo', $daily_operation->produksi_cpo) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">Stok CPO Semalam (MT)</label>
                    <input type="number" step="0.01" name="stok_cpo_yesterday" value="{{ old('stok_cpo_yesterday', $daily_operation->stok_cpo_yesterday) }}" @readonly(!($canEditStokCpoYesterday ?? false)) class="w-full border rounded-lg px-3 py-2 text-sm {{ !($canEditStokCpoYesterday ?? false) ? 'bg-gray-100' : '' }}">
                </div>
                <div>
                ...
                <div id="pk_hopper_wrap" class="hidden">
                    ...
                    <input type="number" step="0.01" name="pk_kcp_to_hopper" value="{{ old('pk_kcp_to_hopper', $daily_operation->pk_kcp_to_hopper ?? 0) }}" min="0" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                ...
                <div id="pk_production_wrap">
                    ...
                    <input id="produksi_pk_input" type="number" step="0.01" name="produksi_pk" value="{{ old('produksi_pk', $daily_operation->produksi_pk) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                ...
                <div id="pk_yesterday_wrap">
                    ...
                    <input type="number" step="0.01" name="stok_pk_yesterday" value="{{ old('stok_pk_yesterday', $daily_operation->stok_pk_yesterday) }}" @readonly(!($canEditStokPkYesterday ?? false)) class="w-full border rounded-lg px-3 py-2 text-sm {{ !($canEditStokPkYesterday ?? false) ? 'bg-gray-100' : '' }}">
                </div>
            </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="oer" value="{{ old('oer', $daily_operation->oer) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">KER (%)</label>
                    <input type="number" step="0.01" name="ker" value="{{ old('ker', $daily_operation->ker) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">FFA (%)</label>
                    <input type="number" step="0.01" name="ffa" value="{{ old('ffa', $daily_operation->ffa) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">Moisture (%)</label>
                    <input type="number" step="0.01" name="moisture" value="{{ old('moisture', $daily_operation->moisture) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-500 mb-1">Dirt (%)</label>
                    <input type="number" step="0.01" name="dirt" value="{{ old('dirt', $daily_operation->dirt) }}" class="w-full border rounded-lg px-3 py-2 text-sm">
                </div>
            </div>
        </div>
        ...
        <div>
            <h3 class="text-sm font-semibold ppnj-green-text mb-3 border-b pb-2">F. Sebab Pindaan</h3>
            <textarea
                ...
                minlength="5"
                maxlength="500"
                placeholder="Nyatakan sebab pindaan dibuat, contohnya pembetulan berdasarkan Daily Figure kilang."
                class="w-full border rounded-lg px-3 py-2 text-sm"
            >{{ old('amendment_reason') }}</textarea>
            <p class="text-xs text-gray-400 mt-1">Ruangan ini wajib diisi untuk semua pindaan rekod harian.</p>
        </div>

        <div class="flex justify-end gap-3 pt-2 border-t">
            <a href="{{ route('rekod-harian.index') }}" class="px-4 py-2 rounded-lg border text-sm">Batal</a>
            <button type="button" id="open-confirmation-modal" class="px-5 py-2 rounded-lg ppnj-green text-white text-sm font-medium">Kemaskini Data</button>
        </div>
    </form>
</div>
    ...
    <div class="relative z-10 min-h-full flex items-end md:items-center justify-center p-0 md:p-4">
        <div class="w-full md:max-w-6xl bg-white rounded-t-2xl md:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col border border-green-100">
            <div class="px-5 md:px-6 py-4 border-b bg-green-50">
                ...
                <p class="text-xs text-gray-600 mt-1">Sila semak semula semua data sebelum pindaan dihantar.</p>
            </div>
            ...
            <div class="px-5 md:px-6 py-4 overflow-y-auto space-y-4 text-sm">
                ...
                <div>
                    <h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2">Sebab Pindaan</h4>
                    <div class="bg-gray-50 rounded-lg p-3"><p class="font-semibold" data-summary="amendment_reason">-</p></div>
                </div>
...
<script>
    ...
    function populateSummary() {
        ...
        setSummaryValue('amendment_reason', getFieldValue('amendment_reason'));
    }
